    ...
    <main className="flex-1 space-y-12 p-4 md:p-12 bg-muted/20 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 border-b border-primary/5 pb-12">
        <div className="space-y-4">
           <div className="flex items-center gap-3">
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.5em] text-primary">Discovery Node: GLOBAL_EX_ALPHA</p>
           </div>
           <h2 className="text-6xl font-black tracking-tight uppercase tracking-tighter leading-[0.8]">Trade <br />Discovery.</h2>
        </div>
        <div className="flex gap-4">
           <Button variant="outline" className="h-16 px-10 border-2 font-black uppercase tracking-widest text-xs bg-background shadow-md" onClick={() => router.push('/discovery/radar')}>
              <Target className="mr-3 h-4 w-4" /> Opportunity Radar
           </Button>
           <Button variant="outline" className="h-16 px-10 border-2 font-black uppercase tracking-widest text-xs bg-background shadow-md" onClick={() => router.push('/marketplace/prices')}>
              <BarChart3 className="mr-3 h-4 w-4" /> Price Index
           </Button>
           <Button className="h-16 px-12 bg-primary text-white font-black uppercase tracking-widest text-xs shadow-3xl hover:scale-[1.02] transition-all" onClick={() => router.push('/buyer/rfqs/new')}>
      ...
      <Tabs defaultValue="discovery" className="space-y-10">
        <TabsList className="bg-background border-2 p-1.5 gap-1.5 h-16 rounded-[24px] shadow-sm w-fit">
          <TabsTrigger value="discovery" className="data-[state=active]:bg-primary data-[state=active]:text-white font-black text-[10px] uppercase h-full px-10 rounded-xl tracking-widest transition-all">Sourcing discovery</TabsTrigger>
          <TabsTrigger value="matchmaking" className="data-[state=active]:bg-primary data-[state=active]:text-white font-black text-[10px] uppercase h-full px-10 rounded-xl tracking-widest transition-all flex items-center gap-2">
            <Sparkles className="h-3.5 w-3.5" /> AI Matchmaking
          </TabsTrigger>
          <TabsTrigger value="signals" className="data-[state=active]:bg-primary data-[state=active]:text-white font-black text-[10px] uppercase h-full px-10 rounded-xl tracking-widest transition-all" onClick={() => router.push('/discovery/signals')}>Market Signals</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="discovery">
          <div className="grid gap-12 lg:grid-cols-12">
               ...
               <div className="flex flex-col lg:flex-row gap-4">
                  <div className="relative flex-1">
                     ...
                     <Input 
                       ...
                       className="pl-14 h-16 bg-background border-2 rounded-[24px] text-lg font-black tracking-tight shadow-inner focus-visible:ring-primary/20"
                       value={search}
                       onChange={(e) => setSearch(e.target.value)}
                     />
                  </div>
                  <Button variant="outline" className="h-16 w-16 border-2 rounded-[24px] shadow-md bg-background shrink-0">
               ...
               <div className="grid gap-8">
                  <AnimatePresence>
                     {filtered.map((listing, i) => (
                       ...
                       >
                         <Card onClick={() => router.push(`/marketplace/listing/${listing.id}`)} className="shadow-xl border-2 hover:border-primary/50 transition-all group overflow-hidden bg-background rounded-[40px] cursor-pointer">
                               ...
                               <div className="flex-1 p-10 flex flex-col md:flex-row items-center justify-between gap-10">
                                  <div className="space-y-6 flex-1 min-w-0">
                                     <div className="flex items-center gap-4">
                                        ...
                                        <span className="text-[10px] text-muted-foreground font-black uppercase tracking-[0.25em] opacity-40">{listing.category} Node</span>
                                     </div>
            ...
            <div className="lg:col-span-4 space-y-12">
               {/* AI ORACLE PANEL */}
               <Card className="shadow-3xl border-none bg-primary text-primary-foreground overflow-hidden relative group rounded-[48px]">
                  ...
                  <CardHeader className="pb-6 border-b border-white/10 relative px-10 pt-10">
                  ...
                  <CardContent className="p-10 relative space-y-12">
                     ...
                     <div className="space-y-6">
                        {matches.slice(0, 3).map((match, i) => (
                           <div key={i} className="p-6 rounded-[24px] bg-white/10 border border-white/10 flex items-center justify-between shadow-inner backdrop-blur-md transition-all hover:bg-white/20 cursor-pointer group/item">
               ...
               <Card className="shadow-none border-2 bg-background p-10 space-y-10 rounded-[48px]">
                  <div className="flex items-center justify-between">
                     <h4 className="text-[11px] font-black uppercase tracking-[0.4em] text-muted-foreground ml-1">Market Equilibrium</h4>
                     <div className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse" />
                  </div>
