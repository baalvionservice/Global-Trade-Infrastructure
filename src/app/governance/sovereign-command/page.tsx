  ...
  if (loading || !pulse) {
    return (
      <div className="flex h-[80vh] flex-col items-center justify-center gap-6 bg-slate-950">
        ...
        <p className="text-[11px] font-black uppercase tracking-[0.5em] text-primary animate-pulse">Establishing Sovereign Link...</p>
      </div>
    );
  }
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 border-b border-white/10 pb-12">
        <div className="space-y-4">
          <div className="flex items-center gap-3">
             ...
             <p className="text-[10px] font-black uppercase tracking-[0.5em] text-primary">Authority Node: SOVEREIGN_KERNEL_ALPHA</p>
          </div>
          <h2 className="text-6xl font-black tracking-tight uppercase tracking-tighter leading-[0.8]">Global <br />Command.</h2>
        ...
        <div className="flex flex-wrap gap-4">
           <div className="flex items-center gap-3 px-8 py-3 bg-white/5 border border-white/10 rounded-2xl text-xs font-black uppercase tracking-widest text-indigo-400 shadow-3xl animate-in zoom-in duration-500">
      ...
      <div className="grid gap-10 lg:grid-cols-12">
           ...
           <CardContent className="p-0 flex-1 overflow-auto custom-scrollbar z-10">
              <div className="divide-y divide-white/5">
                 {pulse.tenants.map((tenant: SovereignTenant) => (
                    <div key={tenant.id} className="p-10 flex items-center justify-between group hover:bg-white/[0.01] transition-colors">
                       <div className="flex items-center gap-10">
                          <div className="h-16 w-16 rounded-[24px] border-2 border-white/10 bg-slate-950 flex items-center justify-center shadow-inner group-hover:scale-105 transition-transform duration-500">
        ...
        <div className="lg:col-span-4 space-y-10">
              ...
              <CardContent className="p-10 relative space-y-10">
                 <div className="space-y-6">
                    {pulse.signals.map((sig: GlobalRiskSignal) => (
                       <div key={sig.id} className="p-6 rounded-[24px] bg-white/10 border border-white/10 backdrop-blur-md space-y-4">
           ...
           <Card className="shadow-none border-none bg-slate-900/40 p-10 space-y-10 rounded-[48px]">
              <div className="flex items-center justify-between">
                 <h4 className="text-[11px] font-black uppercase tracking-[0.4em] text-slate-500">Core Performance</h4>
           ...
           <Card className="shadow-none border-none bg-slate-900/30 p-12 text-center space-y-6 rounded-[48px] border-dashed border-white/5 group hover:border-primary/20 transition-all duration-700">
              ...
              <Button variant="outline" className="w-full h-12 border-white/10 font-black uppercase text-[9px] tracking-[0.2em] bg-slate-900/50 hover:bg-slate-800 text-white">RECONSTRUCT STATE FROM REPLICA</Button>
           </Card>
        </div>
      </div>
    </main>
