  ...
  if (loading && crises.length === 0) {
    return (
      <div className="flex h-[80vh] flex-col items-center justify-center gap-6 bg-slate-950">
        ...
        <p className="text-[11px] font-black uppercase tracking-[0.5em] text-red-500 animate-pulse">ARMING CRISIS COMMAND HANDSHAKE...</p>
      </div>
    );
  }

  return (
    <main className="flex-1 space-y-12 bg-slate-950 text-slate-100 min-h-screen p-4 md:p-12 selection:bg-red-600">
      {/* HEADER */}
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 border-b border-red-500/20 pb-12">
        <div className="space-y-4">
          <div className="flex items-center gap-3">
             <div className="h-2 w-2 rounded-full bg-red-500 animate-ping" />
             <p className="text-[10px] font-black uppercase tracking-[0.5em] text-red-500">Crisis Node: COMMAND_BUNKER_ALPHA</p>
          </div>
          <h2 className="text-6xl font-black tracking-tight uppercase tracking-tighter leading-[0.8] text-white">War Room.</h2>
        ...
        <div className="flex flex-wrap gap-4">
           <div className="flex items-center gap-4 px-8 py-4 bg-red-600 rounded-3xl border-2 border-red-500 font-black text-xs uppercase tracking-widest text-white shadow-4xl animate-pulse">
      ...
      <div className="grid gap-12 lg:grid-cols-12">
        <div className="lg:col-span-8 space-y-12">
           {/* THREAT TOPOLOGY */}
           <Card className="shadow-none border-none bg-red-950/20 rounded-[48px] overflow-hidden flex flex-col h-[550px] relative group border border-red-900/30">
              ...
              <CardContent className="flex-1 flex items-center justify-center p-0 relative">
                 <div className="relative text-center space-y-10">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                       <Crosshair className="h-20 w-20 text-red-600 drop-shadow-[0_0_20px_rgba(239,68,68,0.5)] animate-pulse" />
                    </div>
                    <div className="space-y-2">
                       <p className="text-[11px] font-black uppercase tracking-[0.5em] text-red-400">4 Active Incidents Staged</p>
           ...
           <AnimatePresence>
              {crises.map((crisis, i) => (
                ...
                >
                  <Card className="shadow-4xl border-none bg-red-600/10 rounded-[48px] overflow-hidden ring-1 ring-red-600/30">
                        ...
                        <div className="flex-1 p-12 space-y-10">
                           <div className="flex justify-between items-start">
                              <div className="space-y-4">
                                 <div className="flex items-center gap-6">
                                    <Badge className="bg-red-600 text-white text-[10px] font-black h-7 px-4 border-none shadow-xl tracking-[0.2em] uppercase">{crisis.severity}</Badge>
                           ...
                           <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                              {crisis.impactedClusters.map(node => (
                                 <div key={node} className="p-6 rounded-[24px] bg-white/5 border border-white/10 flex flex-col gap-3 group/node hover:bg-white/10 transition-all">
                           ...
                           <div className="flex flex-col sm:flex-row justify-between items-center gap-10 pt-10 border-t border-white/5">
                              <div className="space-y-2">
                              ...
                              <div className="flex gap-4">
                                 <Button variant="ghost" className="h-16 px-12 border border-white/10 text-white font-black uppercase tracking-widest text-xs hover:bg-white/5 rounded-3xl">
                                    ISOLATE SEGMENTS
                                 </Button>
                                 <Button className="h-16 px-16 bg-white text-red-600 font-black uppercase tracking-widest text-sm shadow-4xl hover:scale-105 transition-all rounded-3xl">
        ...
        <div className="lg:col-span-4 space-y-12">
           ...
           <Card className="shadow-none border-2 border-white/5 bg-slate-900/40 p-10 space-y-12 rounded-[48px]">
              <div className="flex items-center justify-between">
                 <h4 className="text-[11px] font-black uppercase tracking-[0.5em] text-red-500">Incident Command Matrix</h4>
           ...
           <Card className="shadow-4xl border-none bg-primary text-white relative overflow-hidden group rounded-[48px] h-[380px]">
              ...
              <CardContent className="p-10 relative space-y-10">
                 ...
                 <Button variant="secondary" className="w-full h-20 font-black uppercase text-[12px] tracking-[0.4em] shadow-3xl bg-white text-primary border-none rounded-[24px] hover:scale-[1.02] transition-transform">
           ...
           <Card className="shadow-none border-2 bg-background p-12 text-center space-y-8 rounded-[48px] border-dashed group hover:border-primary/20 transition-all duration-700">
              <Compass className="h-16 w-16 mx-auto text-muted-foreground opacity-10 group-hover:text-primary group-hover:opacity-30 transition-all duration-1000 group-hover:rotate-45" />
              <div className="space-y-3">
                 <p className="text-sm font-black uppercase tracking-[0.3em] text-muted-foreground">Recovery Simulation Lab</p>
              ...
              <Button variant="outline" className="w-full h-12 border-2 font-black uppercase text-[9px] tracking-[0.2em] bg-background" onClick={() => router.push(PATHS.GOVERNANCE_SIMULATION)}>OPEN SIMULATION DESK</Button>
           </Card>
        </div>
      </div>
    </main>
